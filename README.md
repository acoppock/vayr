...
it. It introduces variation by randomly sampling points on a rectangle.
This approach is effective but can be unattractive. The position
adjustments in `vayr` aim to do better.
...
`position_jitter_ellipse()` adds elliptical random noise to perfectly
...
`position_sunflower()` arranges perfectly over-plotted points using a
...
of over-plotting, but the density of the pattern remains constant. A
point with nothing over-plotting it stays where it is. We generally
recommend pairing the position adjustment with `coord_equal()`, in which
case the default aspect ratio of 1 yields perfectly circular flowers,
but the aspect ratio of the flowers can be adjusted if need be.
...
keep when the position adjustment is used without `coord_equal()`. The
flowers can be made wider or taller to compensate for the aspect ratio
of the axes or the image. Set `aspect_ratio` to the reciprocal of the
distortion you are correcting: flowers that render twice as wide as they
are tall need an `aspect_ratio` of 0.5.
...
Under `coord_fixed()` the arithmetic is simpler, because `ratio` already
expresses the distortion you need to undo: set `aspect_ratio` to the
same value as `ratio` and the flowers come out circular. The two
parameters agree in value while being defined in opposite directions,
since `aspect_ratio` is width to height and `ratio` is height to width.
The grid below crosses the two, and the circular flowers appear where
the values match, running from the top right to the bottom left.

<img src="man/figures/README-contents_2C-1.png" alt="aspect_ratio" width="100%" />

### Position Circle Pack

`position_circlepack()` uses a circle packing algorithm from
...
parameters. Do not confuse it with `geom_circlepack()` from
`ggcirclepack`, which can be found on
...
inside out in the order of the data. Arranging the data by size
therefore organizes the points accordingly.
...
The figure below visualizes the data using `position_sunflowerdodge()`
